#/bin/bash
#########################
## lk_wifi_db_get_network_clients.sh :
...
#sqlite3 -header $database_name '\
	...
	then
		sql1="$sql1 WHERE SSID.ESSID = \"${essid}\""
		if is_option_present "-t" 
		then
			sql1="$sql1 AND SSID.is_target = 1"
		fi
	else
		if is_option_present "-t" 
		then
			sql1="$sql1 WHERE SSID.is_target = 1"
		fi
	fi
	...
	if is_option_present "-t" 
	then
		sql2="$sql2 AND SSID.is_target = 1"
	fi

	sql="SELECT S.essid as '      ESSID       ', S.client_mac as '   Client MAC    ', S.manuf as '   Manufacturer   ', S.type as ' Client type ' FROM ($sql1 UNION $sql2) as S"
